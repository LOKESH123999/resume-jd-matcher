...
// API Configuration
const API_BASE_URL = "https://resume-jd-matcher-production.up.railway.app";
...
function updateScoreOverview(scoreBreakdown, skills) {
  ...
  document.getElementById("similarityScore").textContent =
    scoreBreakdown.similarity_score.toFixed(1) + "%";

  // Skill score
  document.getElementById("skillScore").textContent =
    scoreBreakdown.skill_match_score.toFixed(1) + "%";

  // Skill ratio
  const matchedCount = skills.matched_skills.length;
  const totalCount =
    skills.matched_skills.length + skills.missing_skills.length;
  document.getElementById("skillRatio").textContent =
...
function updateAIFeedback(aiFeedback) {
  ...
  document.getElementById("fitAnalysis").textContent =
    aiFeedback.fit_analysis || "Analysis not available";
  ...
  if (aiFeedback.skills_to_learn && aiFeedback.skills_to_learn.length > 0) {
    ...
    skillsToLearnDiv.innerHTML =
      '<p class="text-muted">No skill recommendations available</p>';
  }

  // Learning resources
  const resourcesDiv = document.getElementById("learningResources");
  if (
    aiFeedback.learning_resources &&
    aiFeedback.learning_resources.length > 0
  ) {
    ...
    resourcesDiv.innerHTML =
      '<p class="text-muted">No learning resources available</p>';
  }

  // Improvement tip
  document.getElementById("improvementTip").textContent =
    aiFeedback.improvement_tip || "No improvement tip available";
}
